...
//    }
    ...
    public void outputTreeForAllPathwaysToSimpleText(PersistenceAdaptor dba, String fileName, String speciesName) throws Exception {
        EventTreeBuildHelper treeHelper = new EventTreeBuildHelper(dba);
        Collection topEvents = treeHelper.getTopLevelEvents();
        Collection allEvents = treeHelper.getAllEvents();
        treeHelper.loadAttribtues(allEvents);
    ...
    public static void main(String[] args) {
        if (args.length < 7) {
            System.out.println("Usage: java org.gk.database.util.EventTreeXMLOutput dbhost dbName user pwd port outputFileName use_neo4j");
            System.exit(1);
        }
        try {
