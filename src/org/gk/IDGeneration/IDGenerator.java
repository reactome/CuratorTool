...
public class IDGenerator {
    ...
    private PersistenceAdaptor previousDba = null;
    private PersistenceAdaptor currentDba = null;
    private PersistenceAdaptor gk_centraldba = null;
     ...
     */
    public IDGenerator(PersistenceAdaptor previousDba, PersistenceAdaptor currentDba, PersistenceAdaptor gk_centraldba, IdentifierDatabase identifierDatabase) {
    ...
    public IDGenerator(PersistenceAdaptor previousDba, PersistenceAdaptor currentDba, PersistenceAdaptor gk_centraldba, IdentifierDatabase identifierDatabase, PreviousInstanceFinder previousInstanceFinder) {
        this.previousDba = previousDba;
        this.currentDba = currentDba;
        this.gk_centraldba = gk_centraldba;
        this.identifierDatabase = identifierDatabase;
    ...
    public IDGenerator(PersistenceAdaptor previousDba, PersistenceAdaptor currentDba, PersistenceAdaptor gk_centraldba, IdentifierDatabase identifierDatabase, PreviousInstanceFinder previousInstanceFinder, List<Long> limitingCurrentDbIds) {
        this.previousDba = previousDba;
        this.currentDba = currentDba;
        this.gk_centraldba = gk_centraldba;
        this.identifierDatabase = identifierDatabase;
     ...
     * @param classes
     */
    public void generateIDs(List classes, boolean neo4J) {
        generateIDs(classes, false, null, neo4J);
    }
    ...
    public void testForRepeatedStableIdsInIdentifierDatabase() {
        ...
        int stableIdentifierCount = 0;
        try {
            identifierDatabase.getDba().refreshCaches(); // clear cache to avoid heap space problems
     ...
     * @param classes
     */
    public void generateIDs(List classes, List schemaChangeIgnoredAttributes, boolean neo4J) {
        generateIDs(classes, false, schemaChangeIgnoredAttributes, neo4J);
    }
     ...
     */
    public void generateIDs(List classes, boolean testMode, List schemaChangeIgnoredAttributes, boolean neo4J) {
        ...
        for (Iterator it = consolidatedClasses.iterator(); it.hasNext(); ) {
            ...
            generateIDs(currentSchemaClass, testMode, schemaChangeIgnoredAttributes, neo4J);
        }
     ...
     * @param testMode    - if true, nothing will be inserted into database
     */
    public void generateIDs(SchemaClass currentSchemaClass, boolean testMode, List schemaChangeIgnoredAttributes, boolean neo4J) {
        if (currentDba == null) {
        ...
        try {
            ...
            for (Iterator it = instances.iterator(); it.hasNext(); ) {
                ...
                if (instance.getSchemClass().isValidAttribute("species")) {
                    ...
                    System.err.println("");
                generateIDs(instanceBiologicalMeaning, instance, testMode, schemaChangeIgnoredAttributes, neo4J);
            }
        } catch (Exception e) {
...
//			}
            ...
            if (!testMode) {
                PersistenceAdaptor identifierDatabaseDba = IdentifierDatabase.getDba();
    ...
    public void generateIDs(
            InstanceBiologicalMeaning instanceBiologicalMeaning,
            GKInstance currentInstance,
            boolean testMode,
            List schemaChangeIgnoredAttributes,
            boolean neo4J) {
        try {
        ...
        GKInstance previousInstance = previousInstanceFinder.getPreviousInstance(neo4J);
...
//					return;
        ...
        if (previousDba instanceof Neo4JAdaptor) {
            // Neo4J
            Driver driver = ((Neo4JAdaptor) currentDba).getConnection();
            ...
            try {
                ...
                if (currentStableIDs == null || currentStableIDs.size() == 0) {
                    ...
                    currentStableID.setAttributeValue("identifierVersion", stableVersionString);
                    if (!testMode)
                        currentDba.storeInstance(currentStableID, null);
                } else {
                    // Since gk_central now contains stable IDs, they will often
                    // already be present in the slice.  In this case, recycle
                    // the stable IDs found, rather than creating new ones.
                    ...
                    currentStableID.setAttributeValue("identifierVersion", stableVersionString);
                    if (!testMode)
                        currentDba.updateInstanceAttribute(currentStableID, "identifierVersion", null);
                }
        ...
        if (previousDba instanceof Neo4JAdaptor) {
            ...
            try {
                ...
                if (!testMode) {
                    // Update the instance in the current release so that it
                    // contains the appropriate stable ID info.
                    currentDba.updateInstanceAttribute(currentInstance, "stableIdentifier", null);

                    // Update the instance in the current release so that it
                    // contains the appropriate DOI info.
                    if (doi!=null)
                        currentDba.updateInstanceAttribute(currentInstance, "doi", null);
                }
            } catch (Exception e) {
                System.err.println("IDGenerator.generateIDs: WARNING - problem setting up DOI");
                e.printStackTrace(System.err);
                    ...
                    if (gk_centralInstance==null) {
                        ...
                        if (previousInstanceFinder.areInstancesIdentical(currentInstance, gk_centralInstance)) {
                            ...
                            if (gkcentralStableIDs==null || gkcentralStableIDs.size()==0) {
                                ...
                                gkcentralStableID.setAttributeValue("identifier", stableIdentifierString);
                                if (!testMode) {
                                    gk_centraldba.storeInstance(gkcentralStableID, null);
                                }
                            } else {
                                gkcentralStableID = (GKInstance)gkcentralStableIDs.toArray()[0];
                            }
                            ...
                            if (!testMode) {
                                // Make sure the version gets updated.
                                gk_centraldba.updateInstanceAttribute(gkcentralStableID, "identifierVersion", null);
                                // Update the instance in the current release so that it
                                // contains the appropriate stable ID info.
                                gk_centraldba.updateInstanceAttribute(gk_centralInstance, "stableIdentifier", null);
                                // Update the instance in the current release so that it
                                // contains the appropriate DOI info.
                                if (doi!=null)
                                    gk_centraldba.updateInstanceAttribute(gk_centralInstance, "doi", null);
                            }
                        } else {
                            System.err.println("IDGenerator.generateIDs: WARNING - gk_central has no valid instance corresponding to DB_ID=" + currentInstance.getDBID());
                        }
     ...
     * @param classes
     */
    public void testIDs(List classes, boolean neo4J) {
        generateIDs(classes, true);
    }
    ...
    public void rollbackIDs(GKInstance currentInstance) {
        if (currentDba instanceof Neo4JAdaptor) {
            // Neo4J
            Driver driver = ((Neo4JAdaptor) currentDba).getConnection();
            try (Session session = driver.session(SessionConfig.forDatabase(currentDba.getDBName()))) {
                Transaction tx = session.beginTransaction();
                ...
                ((Neo4JAdaptor) currentDba).deleteInstance(currentStableID, tx);
            ...
            try {
                ...
                // Delete the StableIdentifier instance from the Reactome database
                ((MySQLAdaptor) currentDba).deleteInstance(currentStableID);
