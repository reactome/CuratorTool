...
public class IDGenerationCommandLine {
    ...
    private boolean neo4J = false; // If true, run against Neo4J DB; otherwise run against MySQL
    ...
    public void run() {
        ...
        PersistenceAdaptor identifierDbParamsDba = identifierDbParams.getDba(isNeo4J());
        ...
        PersistenceAdaptor previousDba = identifierDatabase.getReleaseDbaFromReleaseNum(previousReleaseNum, projectName, neo4J);
        ...
        idGenerator.generateIDs(cleanClasses, testMode, schemaChangeIgnoredAttributes, neo4J);
    ...
    private void setCurrentReleaseParams() {
        ...
        PersistenceAdaptor instanceDatabaseDba = IdentifierDatabase.getDba();
        ...
        if (instanceDatabaseDba instanceof Neo4JAdaptor) {
            // Neo4J
            Driver driver = ((Neo4JAdaptor) instanceDatabaseDba).getConnection();
...
//			if (currentSliceDbParams.dbName!=null && !currentSliceDbParams.dbName.equals("") && !orthologyMode) {
                ...
                if (releases.size() > 0)
                    ((Neo4JAdaptor) instanceDatabaseDba).updateInstance(release, tx);
                else
...
//			if (currentSliceDbParams.dbName!=null && !currentSliceDbParams.dbName.equals("") && !orthologyMode) {
                    ...
                    instanceDatabaseDba.storeInstance(sliceDbParams, null);
                    release.setAttributeValue("sliceDbParams", sliceDbParams);
                }
                if (currentReleaseDbParams.dbName != null && !currentReleaseDbParams.dbName.equals("")) {
                    releaseDbParams = new GKInstance();
                    ...
                    instanceDatabaseDba.storeInstance(releaseDbParams, null);
                    release.setAttributeValue("releaseDbParams", releaseDbParams);
                }
                ...
                // These are new instances, so store them.
                if (releases.size() > 0)
                    ((MySQLAdaptor) instanceDatabaseDba).updateInstance(release);
                else
                    instanceDatabaseDba.storeInstance(release, null);
            } catch (Exception e) {
                handleError("Problem setting database parameters for the current release");
            }
        }
    ...
    static private void printHelp() {
        ...
        System.err.println(" -neo4J Use Neo4J database instead of MySQL");
    ...
    static public void main(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            } else if (s.equals("-neo4J")) {
                iDGenerationCommandLine.setNeo4J(true);
            } else
