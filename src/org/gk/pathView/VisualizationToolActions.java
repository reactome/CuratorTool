...
public class VisualizationToolActions {
    ...
    private void compareReactionsInSky() {
        ...
        model.setPersistenceAdaptor(tool.getDba());
    ...
    private void compareToAnotherDB() {
        ...
        PersistenceAdaptor dba = tool.getDba();
        ...
        if (connectionPane.showInDialog(tool)) {
            ...
            PersistenceAdaptor oldAdaptor;
            try {
                if (Boolean.parseBoolean(prop.getProperty("neo4J"))) {
                    oldAdaptor = new Neo4JAdaptor(dbHost,
                            dbName,
                            dbUser,
                            dbPwd,
                            Integer.parseInt(dbPort));
                } else {
                    oldAdaptor = new MySQLAdaptor(dbHost,
                            dbName,
                            dbUser,
                            dbPwd,
                            Integer.parseInt(dbPort));
                ...
                compareBrowserPane.setPersistenceAdaptors(oldAdaptor, tool.getDba());
                // Have to make tree building explicitly
                compareBrowserPane.oldPane.setPersistenceAdaptor(oldAdaptor, true);
